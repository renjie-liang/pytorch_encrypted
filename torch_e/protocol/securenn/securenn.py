...
from torch_e.player import Player
from torch_e.protocol import memoize, nodes
from torch_e.protocol.securenn.odd_tensor import oddint64_factory
from torch_e.protocol.pond import Pond, PondTensor, PondPublicTensor, PondPrivateTensor, PondMaskedTensor, _type
from torch_e.tensor import native_factory, int64factory
from torch_e.tensor.factory import AbstractFactory, AbstractTensor
...
# 
	...
	c_const = torch.tensor(c_edge_vals, dtype=prime_dtype.native_type)
	c_const = c_const.reshape([1, bit_length])
...
def _im2col(prot: Pond, x: PondTensor, pool_size: Tuple[int, int],
	...
	# print('_im2col')
	# print((batch * channels, 1, height, width))
	# input()
	#with tf.device(prot.server_0.device_name):
	...
	# print('_im2col')
	# print(y_on_0.shape)
	# print([out_height, out_width, int(batch), int(channels)])
	# input()
	#with tf.device(prot.server_1.device_name):
